...
{
	memcpy(buf, __img_unikraft_start + image_load_offset, bufsize);
	return 0;
}

static ssize_t get_image_size(void *image_info, uint64_t *size)
{
	*size = __img_unikraft_end - __img_unikraft_start;
	return 0;
}
...
struct xen_domain_cfg domd_cfg = {
	...
	.iomems = domd_iomems,
	.nr_iomems = 0 /* sizeof(domd_iomems) / sizeof(*domd_iomems) */,
